...
def plot_images_from_folder(
    ...
    if not isinstance(figsize, tuple) or len(figsize) != 2 or not all(isinstance(x, (int, float)) for x in figsize):
    ...
    image_files = sorted(
        [f for f in os.listdir(folder_path) if f.endswith(img_format)]
    )
...
def plot_images_from_list(
    ...
    :param pathlist: List of file paths to images.
    ...
    for i in range(0, total_imgs, stepsize):
        ...
        to_plot_file_paths = pathlist[i:i + stepsize]
        if verbose:
            print(f"Plotting batch of {len(to_plot_file_paths)} images...")
