...
def plot_df_cols(
    ...
    plot_type: str = "line", # "line", "bar", or "box"
    hue: str | None = None,
    ...
    line_width: float = 2.0, # line specific
    line_styles: Sequence[str] | None = None, # line specific
    markers: Sequence[str] | None = None, # line specific
    marker_size: float = 5.0, # line specific
    box_linewidth: float = 1.5, # box specific
    showfliers: bool = True, # box specific
    show_grid: bool = True,
    ...
    # Determine palette logic
    actual_hue = hue if hue else "Metric"
    num_colors = len(plot_df[hue].unique()) if hue else len(y_cols)
    palette = colors if isinstance(colors, list) else sns.color_palette(colors, n_colors=num_colors)

    # 3. Plot Execution
    if plot_type == "bar":
        sns.barplot(data=melted_df, x=x, y="Value", hue=actual_hue, palette=palette, ax=ax,
                    linewidth=box_linewidth,  edgecolor=".2")
    
    elif plot_type == "box":
        sns.boxplot(data=melted_df, x=x, y="Value", hue=actual_hue, palette=palette, 
                    showfliers=showfliers, linewidth=box_linewidth, ax=ax)
        ...
        for i, metric in enumerate(unique_metrics):
            ...
            sns.lineplot(
                data=subset, x=x, y="Value", label=metric, ax=ax,
                color=palette[i] if not hue and i < len(palette) else None,
                linewidth=line_width, linestyle=style, marker=marker, markersize=marker_size
            )

    # 4. Formatting (Universal)
    ...
    if y_lim is not None:
        ax.set_ylim(y_lim) if isinstance(y_lim, tuple) else ax.set_ylim((0, y_lim))
    ...
    if remove_spines:
        for spine in remove_spines:
            if spine in ax.spines: ax.spines[spine].set_visible(False)
    ...
    if x_tick_rotation != 0:
        ...
        ax.tick_params(axis="x", labelsize=xlabel_fontsize, rotation=x_tick_rotation)

    # Legend Logic
    if show_legend:
        if legend_outside:
            plt.legend(bbox_to_anchor=(1.05, 1), loc="upper left", title=actual_hue)
            plt.tight_layout(rect=[0, 0, 0.85, 1])
        else:
            plt.legend(title=actual_hue)
            plt.tight_layout()
    else:
